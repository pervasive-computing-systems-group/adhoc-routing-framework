...
LineOfSight::LineOfSight(string orbitFileName) {
	homeID = -1;
...
void LineOfSight::pullOrbitData() {
	// This function pulls a text file with each s/c's orbit position over time and some other useful info
	FILE* orbitFile;
	orbitFile = fopen(this->fileName.c_str(), "r");				// open text file
	if (orbitFile == NULL) {
		cerr << "Error in opening file: " << this->fileName << endl;	// print error message if file wasn't opened
	}
	...
	fscanf(orbitFile, "%i" "%i" "%i", &numSC, &timestepSize, &timestepNum);	//read top line of txt file that has key simulation info

	if(LOS_DEBUG){
		cout << "Number of S/C is: " << numSC << endl;
		cout << "Timestep size is: " << timestepSize << endl;
		cout << "Number of timesteps is: " << timestepNum << endl;
	}

	// initialize orbitData and size it
	for (size_t i = 0; i < numSC + 1; i++){
		orbitData[i].resize(timestepNum);	// make each key, i.e. each s/c
	}
...
void LineOfSight::setSCID(int id) {
	homeID = id;
}

/*
 *  Determines if this S/C has Line-Of-Sight with the given S/C, otherScID, at lapsed
 *   time, timeLapsedS, from start of orbits (in seconds).
 *
 ...
 */
bool LineOfSight::isThereLOS(int otherScID, double timeLapsedS)
{
	if(otherScID == homeID) {
		return true;
	}
	...
	int timeStep_Below = timeLapsedS / timestepSize;		// timestep corresponding to t_1
	...
	// X coordinate
	r1[0] = positionBelow[0] + (timeLapsedS - t_1) * (positionAbove[0] - positionBelow[0]) / (t_2 - t_1);
	// Y coordinate
	r1[1] = positionBelow[1] + (timeLapsedS - t_1) * (positionAbove[1] - positionBelow[1]) / (t_2 - t_1);
	// Z coordinate
	r1[2] = positionBelow[2] + (timeLapsedS - t_1) * (positionAbove[2] - positionBelow[2]) / (t_2 - t_1);
	...
	// linearly interpolate between t_1 and t_2 for x, y, and z coordinates
	// X coordinate
	r2[0] = positionBelow[0] + (timeLapsedS - t_1) * (positionAbove[0] - positionBelow[0]) / (t_2 - t_1);
	// Y coordinate
	r2[1] = positionBelow[1] + (timeLapsedS - t_1) * (positionAbove[1] - positionBelow[1]) / (t_2 - t_1);
	// Z coordinate
	r2[2] = positionBelow[2] + (timeLapsedS - t_1) * (positionAbove[2] - positionBelow[2]) / (t_2 - t_1);
	...
	{
		if ((sqrt(r1[0] * r1[0] + r1[1] * r1[1] + r1[2] * r1[2]) > asteroidRadius)
				&& (sqrt(r2[0] * r2[0] + r2[1] * r2[1] + r2[2] * r2[2]) > asteroidRadius))
		{
			if(LOS_DEBUG)
...
void LineOfSight::scalePositions(float scaleFactor)
{// Scale the asteroid radius as well
	asteroidRadius *= scaleFactor;	

	for (int i = 0; i < numSC+1; i++)				// scale positions for each s/c
	{	
		for (int j = 0; j < timestepNum; j++) {	// scale positions at each timestep
